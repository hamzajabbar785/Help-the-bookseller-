// A bookseller has lots of books classified in 26 categories labeled A, B, ... Z. Each book has a code c of 3, 4,
// 5 or more characters. The 1st character of a code is a capital letter which defines the book category.
...
// For the lists L and M of example you have to return the string (in Haskell/Clojure/Racket/Prolog a list of 
// pairs):
...
const stockList = (listOfArt, listOfCat) => {
    ...
    for(let el of listOfArt){
        let letter = el.charAt(0)
        if(obj[letter]){
            obj[letter] += Number(el.split(" ")[1])
        }else{
            obj[letter] = Number(el.split(" ")[1])
        }
    }
...
const stockList = (listOfArt, listOfCat) => {
    return (!listOfArt.length || !listOfArt.length) ? "" : listOfCat.map((w) => {
        let required = listOfArt.filter((el) => el.charAt(0) === w)
        let count = required.reduce((a,b) => {
            return a + Number(b.split(" ")[1])
        },0)
